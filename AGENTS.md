...
- **10-bit / HDR desktops (deep-color capture).** A desktop running 10-bit color or HDR is scanned out by the compositor as a 16-bit-per-channel framebuffer (e.g. `DRM_FORMAT_ABGR16161616`, fourcc `0x38344241`). The in-tree `kmsgrab` demuxer only knows a fixed table of DRM formats and rejects these with `Framebuffer pixel format 38344241 is not a known supported format`. The kmsgrab backend (`video_linux.go` + `drm_formats_linux.go`) handles this: when `OpenInput` fails it probes the primary plane's framebuffer fourcc directly via libdrm (`drmModeGetFB2`); if it is a deep-color format it re-opens kmsgrab claiming a supported 8-bit layout (`bgr0`) and then, per frame, rewrites the frame's DRM descriptor layer format and its frames-context `sw_format` to the matching 16-bit format (`rgba64le`/`bgra64le`) via `patchDRMFrameFormat`. The encoder (`encoder.go`, `setDeepColor`) then inserts a down-conversion stage: `hwdownload,hwupload=derive_device=vaapi,scale_vaapi=format=nv12` for VAAPI, or `hwdownload,format=nv12` for libx264. Normal 8-bit desktops are unaffected.
