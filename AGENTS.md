...
- `server/server.go` — the shared server library: HTTPS server, static/index serving, the unauthenticated `/ca.crt` (`/ca.pem`) endpoint that serves the local CA certificate for client trust-store installation, the unauthenticated `/version` endpoint (`{"version": ...}`, `no-store`) the web client polls on startup and before every reconnect to self-heal by reloading once when the server binary was updated (guarded against reload loops with a sessionStorage flag), virtual-device creation, and version injection. Static serving marks `/sw.js` and `/manifest.json` `Cache-Control: no-cache`: the former so the browser's service-worker update check always reaches the network (the current sw.js self-destructs stale-caching workers registered by older app versions), the latter so installed PWAs do not pin an old manifest; the remaining assets rely on their `?v={{VERSION}}` cache-bust, exposed as `server.Server` with a `New(cli)` / `Run()` / `Shutdown(ctx)` lifecycle (single-use: after `Shutdown`, construct a fresh one). `Run` blocks until the listener fails or `Shutdown` is called. `Shutdown` closes every active `signaling.Session` (tracked in a registry, because `http.Server.Shutdown` does not close hijacked WebSocket connections) so each session's defer chain cleanly stops its FFmpeg capture pipeline — tearing the process down mid-capture, e.g. via an unhandled Ctrl-C, can crash the Wayland compositor — then shuts down the HTTP server and closes the virtual input devices. The `/signal` WebSocket handler upgrades the connection and delegates to a `signaling.Session`, so `server.go` no longer contains WebRTC or device-routing logic.
...
- `video/video_pipewire_linux.go` — Linux pipewire capture backend (Wayland-native screen capture via xdg-desktop-portal + PipeWire). The portal ScreenCast consent flow (CreateSession/SelectSources/Start/OpenPipeWireRemote) lives in `video/portal_linux.go` (pure Go, godbus — permissions are deliberately not persisted, so the desktop shows its share dialog every run); the libpipewire client stream is a thin cgo bridge in `video/pipewire_linux.c`/`pipewire_linux.h` wrapped by `video/pipewire_cgo_linux.go` (thread loop, raw-BGRx/BGRA/RGBx/RGBA/RGB/BGR format negotiation over shared memory). Dequeued PipeWire buffers pass through a bounded drop-oldest queue as zero-copy handles; the capture loop performs the single per-frame memcpy directly into the AVFrame that feeds the filter graph, then returns the buffer to the stream — teardown is two-phase (request-stop, drain, destroy) so no handle can be released into a dead stream. Capture is damage-driven: a static desktop produces no frames; the negotiated delivery fps (adopted over `--video-fps`) and measured inter-frame times drive encoder pacing and sample durations.
- `video/encoder.go` — shared H264 encoder + filter-graph helper (the orthogonal "encoder axis": h264_vaapi / h264_nvenc / libx264), used by all capture backends. Backends feed it custom frames via a `frameMeta` description (dimensions/pixel format/timebase/hardware-frames-context); the FFmpeg input devices build it from their decoded frames, the pipewire backend from the negotiated PipeWire format.
...
- `server/certs.go` — TLS certificate setup: a long-lived local root CA (`ca.crt`/`ca.key`) plus the server leaf (`server.crt`/`server.key`) it signs; CA and leaf reuse, broken-state regeneration, and fingerprint logging. The CA key can mint certificates trusted by every installed client device, so it is mode 0600 and is never served over HTTP. Unit-tested in `server/certs_test.go`.
...
| `--video-source` | `kmsgrab` | Desktop video capture source: `kmsgrab` (DRM framebuffer), `x11grab` (X server), `pipewire` (xdg-desktop-portal + PipeWire; Wayland-native, no CAP_SYS_ADMIN, asks share permission every run), or `none` to disable video. On Windows the source is always `ddagrab` regardless of this flag. With `pipewire` the process drops any setcap-granted file capabilities at startup (see below) |
...
- The **leaf** is **reused across runs** while it is provably still usable (`loadOrGenerateLeaf`): signed by the current CA (so a CA rotation forces a re-sign), inside its validity window with at least a 30-day margin, key-matched, and with SANs (`localhost`, loopbacks, current non-loopback interface IPs) still all present. Any failed check regenerates the leaf (fresh keypair, validity 825 days — the iOS/macOS browser cap); extra SANs in the stored cert (e.g. an interface that went away) do not trigger regeneration. Reusing the leaf keeps the served certificate stable so clients that have NOT installed the CA keep their click-through browser exception across restarts (the exception is keyed to the leaf, not the CA); clients that installed the CA never notice leaf rotation either way. Deleting `server.crt`/`server.key` forces a fresh leaf.
...
The `x11grab` source reads pixels from the X server (no `CAP_SYS_ADMIN` needed) and pairs with any encoder (libx264 software, or h264_vaapi/h264_nvenc via `hwupload`). The `pipewire` source captures via xdg-desktop-portal + PipeWire (the only Wayland-native option; needs a D-Bus session bus, PipeWire and a portal backend for the desktop, e.g. xdg-desktop-portal-gnome/-kde/-wlr; the desktop's share dialog appears each run and the PipeWire stream is negotiated over shared-memory buffers) and, like x11grab, produces software RGB frames that pair with any encoder — libx264 encodes them directly, vaapi/nvenc get them via `hwupload`.

**pipewire and file capabilities.** xdg-desktop-portal identifies D-Bus callers by inspecting `/proc/<pid>/root`; a binary executed with file capabilities (the `setcap cap_sys_admin,cap_dac_override,cap_setpcap=p` grant `make install`/the .deb postinst applies) is kernel-marked non-dumpable, `/proc/<pid>` becomes root-owned, and every ScreenCast request fails with `Portal operation not allowed: Unable to open /proc/<pid>/root`. Restoring dumpability alone does not fix it — the process must also drop its runtime capability sets. Since the pipewire/portal path needs no capabilities, at startup with `--video-source=pipewire` the server calls `dropCapsSoPortalCanIdentifyUs` (server/platform_linux.go): clear all three capability sets, then `prctl(PR_SET_DUMPABLE, 1)`. Side effect: in pipewire mode the caps-based fallback for opening `/dev/uinput` without `input`-group membership is unavailable. Running inara as root has the same portal problem without a fix (skip: the portal can't inspect root's /proc), so pipewire should be run as the session user. On Windows the source is always `ddagrab`, which captures the primary display through Direct3D 11 Desktop Duplication and produces D3D11 hardware frames; it pairs with `h264_nvenc` (via `hwmap=derive_device=cuda`), `h264_amf` (via `scale_d3d11=format=nv12`), `h264_mf` (download to software; the MFT converts and uploads), or `libx264` (software). The source owns the input + decode pipeline (`video_linux.go` / `video_x11grab_linux.go` / `video_pipewire_linux.go` / `video_windows.go`); the shared `encoder.go` owns the filter graph + H264 encoder, chosen from the source's frame type (hardware vs software) and the requested encoder family. The auto encoder default is h264_nvenc on NVIDIA Linux systems (except kmsgrab, which cannot feed nvenc), else h264_vaapi if available, else libx264; on Windows auto resolves to h264_mf (the Media Foundation transform Windows picks for the primary adapter, typically Intel Quick Sync — no GPU vendor detection needed; preferring nvenc/amf over mf is tracked in `docs/TODO.md`).
...
- **Build needs** the FFmpeg/libdrm C dev packages (`libavcodec-dev libavfilter-dev libavformat-dev libavutil-dev libavdevice-dev libdrm-dev`), `libpipewire-0.3-dev` (for the pipewire source), x11grab support in FFmpeg, and CGO. The pipewire backend links `libpipewire-0.3` dynamically (picked up by dpkg-shlibdeps); its cgo flags use fixed include paths (`/usr/include/pipewire-0.3`, `/usr/include/spa-0.2`) instead of pkg-config because the Go tool rejects `libpipewire-0.3.pc`'s `-fno-strict-overflow` cflag. On Windows ddagrab/mf/amf/nvenc and Direct3D 11 are in-tree/in-box — no extra packages.
...
- `github.com/godbus/dbus/v5` — D-Bus client used for the xdg-desktop-portal ScreenCast flow in the pipewire video source.
...
8. With video enabled (default `--video-source=kmsgrab`), switch to the “Tablet” panel and confirm the desktop appears in the `<video>` element. Swipe away to another panel and confirm the video stops rendering; swipe back and confirm it resumes. Try `--video-source=x11grab` (with a software `--video-encoder=libx264`, or auto on a system with VAAPI/NVIDIA) and confirm it captures the X desktop. Try `--video-source=pipewire` on a Wayland session: confirm the desktop's screen-share dialog appears, approve it, and confirm the desktop streams in the tablet panel. Run with `--video-source=none` and confirm the tablet panel shows only the “Tablet” placeholder and trackpad/tablet still work.
