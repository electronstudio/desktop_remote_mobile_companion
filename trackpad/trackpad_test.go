...
func TestDevicePointerMotion(t *testing.T) {
	...
	if err := d.ProcessEvent(Event{Type: "touchstart", T: []Touch{{ID: 1, X: 0.5, Y: 0.5}}}); err != nil {
		t.Fatal(err)
	}

	for i := 0; i < 10; i++ {
		x := 0.5 + float64(i+1)*0.03
		if err := d.ProcessEvent(Event{Type: "touchmove", T: []Touch{{ID: 1, X: x, Y: 0.5}}}); err != nil {
			t.Fatal(err)
		}
		time.Sleep(50 * time.Millisecond)
	}

	if err := d.ProcessEvent(Event{Type: "touchend", T: []Touch{{ID: 1, X: 0.8, Y: 0.5}}}); err != nil {
		t.Fatal(err)
	}
...
func TestDeviceEmitsEvents(t *testing.T) {
	...
	if err := d.ProcessEvent(Event{Type: "touchmove", T: []Touch{{ID: 1, X: 0.6, Y: 0.6}}}); err != nil {
		t.Fatal(err)
	}
	time.Sleep(500 * time.Millisecond)
	...
	if err := d.ProcessEvent(Event{Type: "touchstart", T: []Touch{{ID: 2, X: 0.4, Y: 0.6}}}); err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < 5; i++ {
		...
		if err := d.ProcessEvent(Event{Type: "touchmove", T: []Touch{
			{ID: 1, X: 0.65, Y: y1},
			{ID: 2, X: 0.45, Y: y2},
		}}); err != nil {
			t.Fatal(err)
	...
	if err := d.ProcessEvent(Event{Type: "touchend", T: []Touch{{ID: 2, X: 0.45, Y: 0.35}, {ID: 1, X: 0.65, Y: 0.35}}}); err != nil {
		t.Fatal(err)
	}
	time.Sleep(1 * time.Second)
}
