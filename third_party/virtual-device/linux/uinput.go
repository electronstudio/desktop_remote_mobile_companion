...
// UInputAbsSetup is the uinput_abs_setup struct used with UI_ABS_SETUP.
// It sets the full absinfo for an axis (including resolution) BEFORE the
// device is created, so libinput sees the resolution at probe time.
...
// UI_ABS_SETUP - _IOW('U', 4, struct uinput_abs_setup). Linux 4.16+.
// Used to set absolute axis information (including resolution) before
// UI_DEV_CREATE, so the device is born with the correct capabilities.
var UI_ABS_SETUP = _IOW(UINPUT_IOCTL_BASE, 4, UInputAbsSetup{})
