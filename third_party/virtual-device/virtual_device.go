...
func (vd *virtualDevice) createDevice() (err error) {
	...
	// Set the full absinfo (including resolution) for each absolute axis
	// BEFORE UI_DEV_CREATE using UI_ABS_SETUP (Linux 4.16+). This makes the
	// device appear with the correct resolution at creation time, so libinput
	// sees it during its probe and does not reject tablets with
	// "missing tablet capabilities: resolution".
	//
	// The legacy approach (writing uinput_user_dev above) cannot set the
	// resolution field, and patching it post-create with EVIOCSABS races
	// with libinput's probe — on a cold first run libinput wins and ignores
	// the device for the whole session.
	for _, event := range vd.config.absoluteAxes {
		setup := linux.UInputAbsSetup{
			Code: uint16(event.Axis),
		...
		err = ioctl(vd.fd, linux.UI_ABS_SETUP, uintptr(unsafe.Pointer(&setup)))
		if err != nil {
			return fmt.Errorf("failed to set UI_ABS_SETUP(0x%x): %v", event.Axis, err)
		}
	}
